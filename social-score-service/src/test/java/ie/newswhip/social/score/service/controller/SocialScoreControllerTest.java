...
public class SocialScoreControllerTest {
    ...
    public void setUp() {
        ...
        Mockito.when(socialScoreService.searchSocialScores(ArgumentMatchers.any(
            SearchKey.class),ArgumentMatchers.anyString())).thenReturn(Arrays.asList(google,faceBook));
        ...
        Mockito.doNothing().when(socialScoreService).updateSocialScore(ArgumentMatchers.any(
            SearchKey.class), ArgumentMatchers.anyString(), ArgumentMatchers.anyLong());
    }
...
//    }
    ...
    private String updateRequestAsJsonString() throws Exception {
        ...
        mapper.configure(SerializationFeature.WRAP_ROOT_VALUE, false);
        final ObjectWriter ow = mapper.writer().withDefaultPrettyPrinter();
        final SocialScoreUpdate socialScoreRequest = new SocialScoreUpdate();
        socialScoreRequest.setValue("https://www.duckgo.com/search?query=soemthing");
        socialScoreRequest.setKey(SearchKey.URL);
